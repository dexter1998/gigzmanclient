...
| `node scripts/build-farm-market.mjs` | Rebuild `lib/premium-v2/farm-market.data.ts` from the farmhouse scrape — the dataset every `/farmhouse`, `/estates` and `/land-rates` page is written from |
| `node scripts/cutout-portraits.mjs` | Key Evergreen's team photographs off their green backdrop |
| `node scripts/make-brand-icons.mjs <slug>` | Cut a tenant's favicon set from its own logo |
...
- **Evergreen's listings** originate from dealers Evergreen collaborates with
  and are published with their permission — confirmed by the client on
  2026-09-12, so the inventory itself is not a placeholder. Their
  *photographs* still are: generated images, not the properties described, and
  the detail page carries an illustrative-image notice until real ones arrive.
  The whole farmhouse pSEO family aggregates from this same dataset
  (`lib/premium-v2/farm-market.data.ts`, generated by
  `scripts/build-farm-market.mjs`), so replacing the source data and re-running
  the script updates every one of those pages.
