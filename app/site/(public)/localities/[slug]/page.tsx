...
import { getFirmSettings, getLocality, getProperties, getPropertyImages } from "@/lib/content";
import { formatIndianPrice, formatDate } from "@/lib/format";
import { buildBreadcrumbJsonLd, jsonLdProps } from "@/lib/schema-org";

export async function generateMetadata(props: PageProps<"/site/localities/[slug]">) {
...
export default async function LocalityDetailPage(props: PageProps<"/site/localities/[slug]">) {
  const { slug } = await props.params;
  const tenant = await getTenant();
  if (!tenant) notFound();
