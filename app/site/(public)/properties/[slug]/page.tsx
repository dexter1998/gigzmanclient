...
export async function generateMetadata(props: PageProps<"/site/properties/[slug]">) {
...
export default async function PropertyDetailPage(props: PageProps<"/site/properties/[slug]">) {
  const { slug } = await props.params;
  const tenant = await getTenant();
  if (!tenant) notFound();
  ...
  const [images, sameLocality] = await Promise.all([
    getPropertyImages(property.id),
    property.locality
      ? getProperties(tenant.id, { locality: property.locality })
      : Promise.resolve([]),
  ]);
